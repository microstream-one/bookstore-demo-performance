...
public interface DataMigrator
{
	...
	public static DataMigrator FromMicroStream(
		final ReadMeCorp readMeCorp,
		final Repositories repositories
	)
	{
		return new FromMicroStream(readMeCorp, repositories);
	}
	
	public static class FromMicroStream implements DataMigrator, HasLogger
	{
		...
		static final class BatchInserter<T> implements BatchPreparedStatementSetter
		{
		...
		static final class BatchDumper<T> implements Dumper<T>
		{
			private final Iterator<Entry<T, Long>> iterator;
			private final int                      size;
		...
		@SafeVarargs
		private static <T> Stream<T> concat(
		...
		private void gc()
		{
//			final EmbeddedStorageManager storageManager = this.readMeCorp.storageManager();
// 			storageManager.persistenceManager().objectRegistry().clear();
