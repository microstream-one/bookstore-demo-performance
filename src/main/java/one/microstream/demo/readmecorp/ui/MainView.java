...
import one.microstream.demo.readmecorp.app.Action;
import one.microstream.demo.readmecorp.app.ActionExecutor;
import one.microstream.demo.readmecorp.app.ClearAction;
import one.microstream.demo.readmecorp.app.ExecutionCallback;
import one.microstream.demo.readmecorp.app.QueryAction;
import one.microstream.demo.readmecorp.app.QueryStats;
...
public class MainView extends VerticalLayout implements PageConfigurator, ExecutionCallback
{
	...
	private Component createBanner()
	{
		...
		label.getElement().setProperty("innerHTML", "&bull; ReadMeCorp &bull;");
		label.getStyle()
			.set("font-size", "150%")
	...
	private Component createMicroStreamControl()
	{
		final Component lazyRefsAndObjectCacheControl = this.createClearActionControl(
			ClearAction.LazyRefsAndObjectCache(this.readMeCorp.storageManager())
		);
		final Component storageCacheControl           = this.createClearActionControl(
			ClearAction.StorageCache(this.readMeCorp.storageManager())
		);
